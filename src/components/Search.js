...
import { getSongList, getFavList, getBiliSeriesList, getBiliColleList, getBiliSeasonList } from '../background/DataProcess'
...
export const Search = function ({ handleSeach }) {
    ...
    const keyPress = (e) => {
        ...
        if (e.keyCode == 13) {
            ...
            // for seasonList
            reExtracted = /.*\.com\/(\d+)\/lists\/(\d+).*/.exec(input);
            if (reExtracted !== null) {
                getBiliSeasonList(reExtracted[1], reExtracted[2])
                    .then((songs) => {
                        const list = {
                            songList: songs,
                            info: { title: `搜索合集- 用户${reExtracted[1]}的合集${reExtracted[2]}`, id: ('FavList-' + 'Search') }
